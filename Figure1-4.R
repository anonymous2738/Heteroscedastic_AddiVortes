...
AddiVortes_Algorithm<-function(y,x,m = 200, m_var = 40 ,max_iter = 1200,burn_in= 200,nu = 6,q =0.85,k = 3 ,sd = 0.8 ,Omega = 3,lambda_rate = 25,YTest,XTest,IntialSigma = "Linear",thinning=1){
  ...
  unifdr = runif(1000)
  #graph_values<-qqplot(yquantile,unifdr,plot.it = FALSE)
  #points(graph_values,pch=16)
...
Homo_AddiVortes_Algorithm<-function(y,x,m = 200 ,max_iter = 1200,burn_in= 200,nu = 6,q =0.85,k = 3 ,sd = 0.8 ,Omega = 3,lambda_rate = 25,YTest,XTest,IntialSigma = "Linear",thinning=1){
  ...
  for (i in 1:max_iter){
    ...
    if (i>burn_in ){ 
      ...
      print(Sigma_squared_test)
    }
    
    for (j in 1:m){
      NewTessOutput<-Homo_NewTess(xScaled,j,Tess,Dim,sd) #Propose new Tessellation 
      TessStar<-NewTessOutput[[1]]  
      DimStar<-NewTessOutput[[2]]
      Modification<-NewTessOutput[[3]]
      
      ResidualsOutput<-Homo_CalculateResiduals(yScaled,xScaled,j,SumOfAllTess,Tess,Dim,Pred,TessStar,DimStar,LastTessPred) #Calculate the n-vector of partial residuals derived from a fitting process that excludes the jth tessellation and the number of observations in each cell.
      R_ijOld<-ResidualsOutput[[1]]   #Old and New refer to the original and proposed tessellations
  ...
  yquantile = qsamp(YTest,pdraw)
  unifdr = runif(1000)
  qqplot(yquantile,unifdr,pch=16,col='grey',xlab="Sample Quantile",ylab="Uniform")
  abline(0,1,col='red')
  ...
  return( #Returns the RMSE value for the test samples.
    list(
      ...
      sigma_squared_test=Sigma_squared_test,
      e_stat=test_result,
      pdraw=pdraw
...
Homo_SigmaSquaredCalculation<-function(yScaled,nu,lambda,SumOfAllTess){ #Sample sigma squared from inverse gamma distribution
...
Homo_NewTess<-function(x,j,Tess,Dim,sd){ #Propose a new tessellation
...
Homo_AlphaCalculation<-function(x,Tess,Dim,j,R_ijOld,n_ijOld,R_ijNew,n_ijNew,SigmaSquared,Modification,SigmaSquaredMu,Omega,lambda_rate){ #Calculates the acceptence rate of the proposed tessellation.
...
Homo_CalculateResiduals<-function(y,x,j,SumOfAllTess,Tess,Dim,Pred,TessStar,DimStar,LastTessPred){ #A function that calculates the n-vector of partial residuals derived from a fitting process that excludes the jth tessellation.
  if (j==1){
    indexes=Indexes(x,Tess[[j]],Dim[[j]]);
    CurrentTessPred<-Pred[[j]][indexes]
    SumOfAllTess=SumOfAllTess-CurrentTessPred}
...
Homo_NewPredSet<-function(j,Tess,R_ijNew,n_ijNew,sigmaSquaredMu,SigmaSquared){ #Sampling the new output values for the new tessellation.
  PredSet=rep(0,length(Tess[[j]][,1]))
  for (i in 1:length(Tess[[j]][,1])){
...
Homo_TestPrediction<-function(x,m,Tess,Dim,Pred){ #A function that derives a prediction for the output variable give an observation, for one iteration in AddiVortes.
  Prediction=rep(0,length(x[,1]));
  for (j in 1:m){
    NewTessIndexes=Indexes(x,Tess[[j]],Dim[[j]]);
    Prediction=Prediction+Pred[[j]][NewTessIndexes]
...
, function(x){
  0.5*(10*sin(pi*x[,1]*x[,2])+20*(x[,3]-0.5)^2+20*x[,4]+5*x[,5])
})
...
graph_results<- foreach(i = 1:3) %dopar% {
  library(invgamma)
  library('FNN')
  library(energy)
  ...
  set.seed(25)
  n=length(Y)
  TrainSet=sort(sample.int(n,4*n/8));
  TestSet=1:n;
...
par(mfrow=c(1,3))
par(mgp = c(2.5, 1, 0))
Homo_AddiVortes<-graph_results[[1]][[1]]
plotCI(sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored), Homo_AddiVortes$UpperConfidenceTRAINValue - sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored) - Homo_AddiVortes$LowerConfidenceTRAINValue,sfrac = 0, scol = 'grey',ylab = expression(hat(s) * (bold(x)) ~ "Posterior Intervals"), xlab = expression(hat(s)* (bold(x))), cex.lab = 1,pch = NA)
abline(mean(sqrt(graph_results[[1]][[2]]$sigma_squared_test)),0,lwd=3)
abline(quantile(sqrt(graph_results[[1]][[2]]$sigma_squared_test),0.95),0, lty =2,lwd=3)
abline(quantile(sqrt(graph_results[[1]][[2]]$sigma_squared_test),0.05),0,lty = 2,lwd=3)

Homo_AddiVortes<-graph_results[[2]][[1]]
plotCI(sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored), Homo_AddiVortes$UpperConfidenceTRAINValue - sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored) - Homo_AddiVortes$LowerConfidenceTRAINValue,sfrac = 0, scol = 'grey',ylab = NA, xlab = expression(hat(s)* (bold(x))), cex.lab = 1,pch = NA)
abline(mean(sqrt(graph_results[[2]][[2]]$sigma_squared_test)),0,lwd=3)
abline(quantile(sqrt(graph_results[[2]][[2]]$sigma_squared_test),0.95),0, lty =2,lwd=3)
abline(quantile(sqrt(graph_results[[2]][[2]]$sigma_squared_test),0.05),0,lty = 2,lwd=3)

Homo_AddiVortes<-graph_results[[3]][[1]]
plotCI(sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored), Homo_AddiVortes$UpperConfidenceTRAINValue - sqrt(Homo_AddiVortes$Sigma_squared_stored),sqrt(Homo_AddiVortes$Sigma_squared_stored) - Homo_AddiVortes$LowerConfidenceTRAINValue,sfrac = 0, scol = 'grey',ylab = NA, xlab = expression(hat(s)* (bold(x))), cex.lab = 1,pch = NA)
abline(mean(sqrt(graph_results[[3]][[2]]$sigma_squared_test)),0,lwd=3)
abline(quantile(sqrt(graph_results[[3]][[2]]$sigma_squared_test),0.95),0, lty =2,lwd=3)
abline(quantile(sqrt(graph_results[[3]][[2]]$sigma_squared_test),0.05),0,lty = 2,lwd=3)
...
par(mfrow=c(1,2))
par(mgp = c(2.5, 1, 0))
plotCI(s[[2]](X[graph_results[[2]][[4]],]),sqrt(graph_results[[2]][[1]]$Sigma_squared_stored), graph_results[[2]][[1]]$UpperConfidenceTRAINValue-sqrt(graph_results[[2]][[1]]$Sigma_squared_stored),sqrt(graph_results[[2]][[1]]$Sigma_squared_stored)-graph_results[[2]][[1]]$LowerConfidenceTRAINValue,sfrac=0, scol = 'grey',ylab = expression(hat(s) * (bold(x)) ~ "Posterior Intervals"), xlab = expression("Out-of-Sample" ~ s(bold(x))),  cex.lab = 1,pch =16,col= 'black')
abline(0,1, col = 'red',lwd=5)
abline(mean(sqrt(graph_results[[2]][[2]]$sigma_squared_test)),0,lwd=3)
sqrt(mean((sqrt(graph_results[[2]][[1]]$Sigma_squared_stored)-s[[2]](X[graph_results[[2]][[4]],]))^2))
sqrt(mean((mean(sqrt(graph_results[[2]][[2]]$sigma_squared_test))-s[[2]](X[graph_results[[2]][[4]],]))^2))
abline(quantile(sqrt(graph_results[[2]][[2]]$sigma_squared_test),0.95),0, lty =2,lwd=3)
abline(quantile(sqrt(graph_results[[2]][[2]]$sigma_squared_test),0.05),0,lty = 2,lwd=3)

plotCI(s[[3]](X[graph_results[[3]][[4]],]),sqrt(graph_results[[3]][[1]]$Sigma_squared_stored), graph_results[[3]][[1]]$UpperConfidenceTRAINValue-sqrt(graph_results[[3]][[1]]$Sigma_squared_stored),sqrt(graph_results[[3]][[1]]$Sigma_squared_stored)-graph_results[[3]][[1]]$LowerConfidenceTRAINValue,sfrac=0, scol = 'grey',ylab = NA, xlab = expression("Out-of-Sample" ~ s(bold(x))),  cex.lab = 1,pch =16,col= 'black')
abline(0,1, col = 'red',lwd=5)
abline(mean(sqrt(graph_results[[3]][[2]]$sigma_squared_test)),0,lwd=3)
sqrt(mean((sqrt(graph_results[[3]][[1]]$Sigma_squared_stored)-s[[3]](X[graph_results[[3]][[4]],]))^2))
sqrt(mean((mean(sqrt(graph_results[[3]][[2]]$sigma_squared_test))-s[[3]](X[graph_results[[3]][[4]],]))^2))
abline(quantile(sqrt(graph_results[[3]][[2]]$sigma_squared_test),0.95),0, lty =2,lwd=3)
abline(quantile(sqrt(graph_results[[3]][[2]]$sigma_squared_test),0.05),0,lty = 2,lwd=3)

plotCI(f(X[graph_results[[2]][[4]],]),graph_results[[2]][[1]]$mean_yhat_Test_stored,graph_results[[2]][[1]]$UpperConfidenceTESTValue_stored-graph_results[[2]][[1]]$mean_yhat_Test_stored,graph_results[[2]][[1]]$mean_yhat_Test_stored-graph_results[[2]][[1]]$LowerConfidenceTESTValue_stored,sfrac=0, scol = 'grey', ylab = expression(hat(f) * (bold(x)) ~ "Posterior Intervals"), xlab = expression("Out-of-Sample" ~ f(bold(x))), cex.lab = 1, pch =16)
abline(0,1,col = 'red',lwd=5)
sqrt(mean((graph_results[[2]][[1]]$mean_yhat_Test_stored-f(X[graph_results[[2]][[4]],]))^2))

plotCI(f(X[graph_results[[3]][[4]],]),graph_results[[3]][[1]]$mean_yhat_Test_stored,graph_results[[3]][[1]]$UpperConfidenceTESTValue_stored-graph_results[[3]][[1]]$mean_yhat_Test_stored,graph_results[[3]][[1]]$mean_yhat_Test_stored-graph_results[[3]][[1]]$LowerConfidenceTESTValue_stored,sfrac=0, scol = 'grey', ylab = NA, xlab = expression("Out-of-Sample" ~ f(bold(x))), cex.lab = 1, pch =16)
abline(0,1,col = 'red',lwd=5)
sqrt(mean((graph_results[[3]][[1]]$mean_yhat_Test_stored-f(X[graph_results[[3]][[4]],]))^2))
