...
if (!requireNamespace("hbart", quietly = TRUE)) {
  library(devtools)
...
### cars dataset ###
{
  ...
  df <- as.data.frame(car_data, stringsAsFactors = TRUE)
  
  # Set appropriate column names
  colnames(df) <- c("price", "trim", "isOneOwner", "mileage", "year", "color", "displacement")
  
  # Convert necessary columns to numeric (if they are numerical)
  ...
  encoded_df <- model.matrix(~ trim + isOneOwner + color + displacement - 1, data = df)
  X<-cbind(df$mileage,df$year,encoded_df,ifelse(df$color=='Black',1,0),ifelse(df$displacement=='4.6',1,0))
  Y<-df$price
}
...
hyperparametersHBART <- expand.grid(
  m=c(200),
  k=c(0.25,0.5,1,2,5,10,20)
)

# Define the paired values for nu and q
nu_q_pairs <- data.frame(nu = c(3,6,10), q = c(0.75,0.85,0.95))

# Use expand.grid for the other hyperparameters and then combine with nu_q_pairs
...
# Merge the grid with nu_q_pairs to keep nu and q as pairs
hyperparametersAddiVortes <- merge(hyperparametersAddiVortes, nu_q_pairs)
...
for (l in 1:1){
  ...
  n=length(Y)
  TrainSet<-matrix(nrow=trunc(4*n/5),ncol = NumOfRep)
  TestSet<-matrix(nrow=n-trunc(4*n/5),ncol = NumOfRep)
  
  for (h in 1:NumOfRep){
    TrainSet[,h]=sort(sample.int(n,4*n/5));
    TestSetInit<-1:n
  ...
  results_matrixAddiVortes <- matrix(ncol = 19, nrow = nrow(hyperparametersAddiVortes))
  ...
  #results_matrixAddiVortes_ordered<-results_matrixAddiVortes[order(results_matrixAddiVortes[, 14]),]
  
  #boxplot.matrix(results_matrixAddiVortes_ordered[1:18, 9:13],use.col=FALSE)
  #boxplot.matrix(results_matrixAddiVortes_ordered[1:36, 15:19],use.col=FALSE)
  ...
  # Iterate over hyperparameter combinations
  results_matrixHomoAddiVortes<-foreach(i = 1:nrow(hyperparametersAddiVortes), .combine = rbind) %dopar% {
    library('FNN')
    library('invgamma')
    library("energy")
    library('scoringRules')
    ...
    for (j in 1:num_folds) {
      ...
      # Make predictions on the test set
      model_AddiVortes<-Homo_AddiVortes_Algorithm(Y[TrainSetCV],as.matrix(X[TrainSetCV,]),m,1200,200,nu = nu,q =q,k,sd,omega,lambda,Y[TestSetCV],as.matrix(X[TestSetCV,]))
      e_stat_value[j] <- model_AddiVortes$e_stat
      #crps_value[j]<-model_AddiVortes$crps_val
      RMSE_value[j]<-model_AddiVortes$Out_of_sample_RMSE
    }                   
  ...
  # Find the best hyperparameter combination based on auc
  best_index <- which.min(results_matrixHomoAddiVortes[, 8])
